...
{
    [Conditional("DEV_VER")]
    public static void Log(string msg) // 컨디셔널 속성의 기능 : 조건부 컴파일 심볼
    {
        //현재 시간을 날짜와 시간의 형식으로 표현해 {0}에 넣고, 로깅 하려는 메세지를 {1}에 넣는다.
        UnityEngine.Debug.LogFormat("[{0}]{1}", System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff"), msg);
    }
    [Conditional("DEV_VER")]
    public static void LogWarning(string msg) // 워닝 로그 함수
    {
        UnityEngine.Debug.LogWarningFormat("[{0}]{1}", System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff"), msg);
    }
    public static void LogError(string msg) // 에러 로그 함수
    {
